...
def convert_all_svg_to_png(svg_dir, output_dir, width=300):
    """Konverterer alle SVG-filer til PNG med spesifisert bredde, beholder aspect ratio"""
    ...
    print(f"📏 Bredde: {width}px (beholder aspect ratio)")
    print("-" * 50)
    ...
    for i, svg_file in enumerate(svg_files, 1):
        ...
        try:
            # Bruk svgexport med kun bredde for å beholde aspect ratio
            cmd = [
                'svgexport',
                str(svg_file),
                str(png_file),
                f'{width}:'  # Kun bredde, høyde beregnes automatisk
            ]
...
def verify_flag_widths(output_dir, expected_width=300):
    """Verifiserer at alle flagg har riktig bredde"""
    print(f"🔍 Verifiserer flagg-bredder i {output_dir}/")
    ...
    try:
        ...
        print("⚠️  PIL ikke tilgjengelig, kan ikke verifisere bredder")
        return
        ...
        try:
            with Image.open(png_file) as img:
                ...
                if width == expected_width:
                    ...
                    wrong_width += 1
                    print(f"❌ {png_file.name}: {width}x{height} (forventet bredde: {expected_width})")
        except Exception as e:
            wrong_width += 1
            print(f"❌ {png_file.name}: feil - {e}")
    
    print(f"\n📊 Bredde-verifikasjon:")
    print(f"✅ Riktig bredde ({expected_width}px): {correct_width}")
    print(f"❌ Feil bredde: {wrong_width}")
    print(f"📈 Nøyaktighet: {correct_width/(correct_width+wrong_width)*100:.1f}%")

def main():
    """Hovedfunksjon som kjører hele prosessen"""
    
    svg_dir = "temp_flags/svg"
    output_dir = "flags_new_300px"
    width = 300
    ...
    print("🚀 STARTER KOMPLETT FLAGGOPPGRADERING (300px BREDDE)")
    print("=" * 60)
    
    # Steg 1: Backup gamle flagg
    backup_dir = backup_old_flags()
    
    # Steg 2: Konverter alle SVG til PNG med 300px bredde
    successful, failed = convert_all_svg_to_png(svg_dir, output_dir, width=width)
    
    if successful == 0:
        print("❌ Ingen flagg ble konvertert. Avbryter.")
        sys.exit(1)
    
    # Steg 3: Verifiser bredder
    verify_flag_widths(output_dir, expected_width=width)
    ...
    print("=" * 60)
    print(f"✅ {successful} nye flagg konvertert (alle {width}px brede)")
